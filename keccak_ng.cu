...
// ---------------------------------------------------------------------------
// HashToken mining kernel, next-gen. Adds a second, far easier "share" target
// so a rig can PROVE it is hashing.
//
// Everything below the share block is byte-for-byte the original. The share
// check costs ~3 registers because it compares only the TOP 64 BITS against a
// 64-bit target, rather than running a second 32-byte byte-wise loop. That
// matters: the unmodified kernel already uses 80 registers with 0 spills, and
// block=1024 already fails to launch (1024 x 80 = 81,920 > the 65,536 limit).
// At the chosen block=256 we have room to 256 regs/thread, so 83 is safe.
//
// Shares are emitted as (base_nonce_part, thread_id) pairs -- 16 bytes instead
// of a 32-byte nonce -- into a ring buffer indexed by atomicAdd. atomicAdd
// returns the PRE-increment value, so d_share_count is the true number of hits
// even when the buffer clips; the host can then tell "buffer overflowed" from
// "this rig is lying".
// ---------------------------------------------------------------------------
extern "C" __global__ void find_solution_kernel(const unsigned char* d_prev_hash,
    ...
    *(unsigned long long*)(&current_nonce[8]) = thread_id;
    ...
    // --- SHARE PATH -------------------------------------------------------
    // hash_result[0] is the most-significant byte in the same big-endian sense
    // the real-target loop below uses, so this is a strict PREFIX of that
    // comparison: every real solution is also a share. That gives a free
    // redundant channel for the thing that actually pays.
    unsigned long long h_hi = 0;
    #pragma unroll
    for (int k = 0; k < 8; ++k) {
        h_hi = (h_hi << 8) | (unsigned long long)hash_result[k];
    }
